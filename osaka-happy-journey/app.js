...
function renderHome(){
  ...
  $('#homeView').innerHTML=`
    <section class="hero"><img class="cover-img" src="images/osaka.svg" alt="Osaka travel illustration"><div class="kicker">OUR JAPAN TRIP</div><h2>${esc(lead)}</h2><p>${esc(sub)}</p>
...
function renderTrip(){
 ...
 <article class="card day-card"><img class="day-cover" src="${dayImage(d)}" alt="${esc(d.title)}"><div class="day-head"><div class="day-emoji">${d.emoji}</div><div><span class="badge gray">${d.day} · ${fmtDate(d.date)}</span><h3>${esc(d.title)}</h3><div class="day-meta">${esc(d.city)} · อาหาร ${esc(d.meals)}</div></div></div><p>${esc(d.summary)}</p>
...
function renderMore(){
 ...
 <div class="cards">
 <div class="card"><img class="day-cover" src="images/food.svg" alt="Osaka food illustration"><h3>🍜 Food & Travel Toolkit</h3><p>V2 เพิ่ม Phrasebook, อ่านออกเสียง, Near My Plan และแชร์ Free Day ระหว่างสองเครื่อง</p></div>
 ...
 </div><p class="footer-note">ข้อมูลเวลาร้าน ราคา และเส้นทางรถไฟควรเช็ก Google Maps/หน้าร้านอีกครั้งใกล้วันเดินทาง</p>`;
}
function setupPWA(){
